...
const TransactionForm = ({
  ...
  const validationSchema = Yup.object({
    ...
    sourceDestination: Yup.string()
      .required('Source/Destination is required')
  ...
  const filteredCategories = Array.isArray(categories)
    ? categories
        .filter((cat) => String(cat.type).toLowerCase() === formik.values.type)
        .map((cat) => ({
  ...
  useEffect(() => {
    ...
    if (formik.values.type && Array.isArray(categories) && categories.length > 0) {
      const currentCategoryIsValidForType = filteredCategories.some(
        (cat) => cat.value === formik.values.category
      );

      if (!currentCategoryIsValidForType) {
  ...
  return (
    <form onSubmit={formik.handleSubmit} className='space-y-4 md:space-y-6'>
      ...
      <Select
        ...
        value={
          filteredCategories.find((opt) => opt.value === String(formik.values.category)) || null
        }
        onChange={(option) => {
      ...
      <Input
        ...
        label={
          formik.values.type === 'income'
            ...
            : 'Merchant/Destination (e.g., Swiggy, Rent)'
        }
